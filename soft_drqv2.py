...
############################################################################################
        ...
        self.log_alpha = torch.tensor(np.log(self.init_temperature)).to(self.device)    ###zzh
        #self.learnable_temperature = learnable_temperature

        #self.action_dim = action_dim
        #self.target_entropy = -self.action_dim ##set target entropy to -|A|
############################################################################################
...
############################################################################################
    ...
    def load_model(self, encoder = False, actor = True, critic = False, critic_target = False): ##choose which parameters load here
        ...
        if encoder:
            self.encoder = torch.load(f'{root_dir}/model_encoder/_model_encoder'+f'_Epochs_{epochs}_.pt')       ###zzh
...
############################################################################################
        actor_loss = (self.alpha.detach() * log_prob - Q).mean()   ###zzh
############################################################################################
...
############################################################################################
        ...
        if self.learnable_temperature:
            ...
            alpha_loss = (self.alpha * (-log_prob - self.target_entropy).detach()).mean()   ###zzh
...
############################################################################################
    ...
    def update(self, replay_iter, step):
        ...
        #########################################################################################
        ######## Save models here, load models in visual.py ###################
        root_dir='/home/henry/Desktop/model_parameters/'+f'{self.task_name}'  #+'_transfer' ##transfer learning
        if step % 50000 == 0:
